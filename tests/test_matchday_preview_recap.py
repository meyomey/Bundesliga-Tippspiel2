...
def test_matchday_preview_page(client, db, user, competition, teams):
    ...
    assert 'Spieltags-Vorschau'.encode('utf-8') in resp.data
    assert b'2:1' in resp.data
    # Mini-Chart (Tippquote) und deutscher Status vorhanden
    assert b'previewQuoteChart' in resp.data
    assert 'geplant'.encode('utf-8') in resp.data


def test_matchday_recap_page(client, db, user, competition, finished_match):
    ...
    assert resp.status_code == 200
    assert 'Spieltags-Rückblick'.encode('utf-8') in resp.data
    assert user.username.encode() in resp.data
    assert b'4' in resp.data
    # Mini-Chart (Punkteverteilung) vorhanden
    assert b'recapPunkteChart' in resp.data
